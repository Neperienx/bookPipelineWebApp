...
# Map a bunch of common Unicode punctuation/spaces to Latin-1 friendly equivalents.
...
def _safe_multi_cell(pdf: FPDF, width: float, height: float, text: str) -> None:
    ...
    try:
        ...
        # Defensive retry: move to next line and try again.
        pdf.ln(height)
        pdf.set_x(pdf.l_margin)
        try:
            pdf.multi_cell(width, height, sanitized)
...
def export_chapter_drafts_to_pdf(
    ...
    *,
    ...
    pdf = FPDF()  # defaults: portrait, mm, A4
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.set_left_margin(15)
    pdf.set_right_margin(15)

    # Use the attribute that both fpdf2 and pyfpdf expose for cell padding:
    # In both, `c_margin` controls the internal horizontal padding for cells.
    # (No method call needed; some fpdf2 versions don't have set_cell_padding.)
    pdf.c_margin = 1  # type: ignore[attr-defined]

    # Effective printable width: prefer pdf.epw if present (fpdf2), else compute.
    ...
    resolved_path = Path(output_path) if output_path else Path("temp.pdf")
